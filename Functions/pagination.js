...
const pagination = async (message, embeds, numberOfPages, numberOfItems, footer, icon, snipe) => {
  embeds[0].setFooter({ text : `${embeds[0].footer ? embeds[0].footer.text.replace('{page}', `1`) + ',' : ''}`, iconURL : icon || null });
  const row = new MessageActionRow().addComponents(
    //new MessageButton().setCustomId("last").setLabel('<:last:968145984221749298>').setStyle("SECONDARY"),
    new MessageButton().setCustomId("prev").setLabel('Left').setStyle("PRIMARY"),
    //new MessageButton().setCustomId("cancel").setLabel('⏹️').setStyle('PRIMARY'),
    new MessageButton().setCustomId("next").setLabel('Right').setStyle("PRIMARY"),
    //new MessageButton().setCustomId("first").setLabel('<:first:968145984100114442>').setStyle("SECONDARY"),
    new MessageButton().setCustomId("skip").setLabel('Fast Forward').setStyle("SECONDARY"),
  );
  ...
  collector.on("collect", async (interaction) => {
    ...
    if (interaction.customId == "prev") {
      ...
      embeds[index].setFooter({ text : `${embeds[index].footer ? embeds[index].footer.text.replace('{page}', `${index}`) : ''}`, iconURL : icon || null });
