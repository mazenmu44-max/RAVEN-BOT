...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        pagedData.forEach((page) => {
            ...
            const listEmbed = new MessageEmbed().setAuthor({ name: `${message.member.displayName}`, iconURL: message.author.displayAvatarURL({ dynamic: true }) }).setTitle(`List of Members`).setDescription(`${items}`).setColor(message.member.displayHexColor)
