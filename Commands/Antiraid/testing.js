...
module.exports = {
    ...
    run : async (client, message, args) => {
        ...
        let msg = ''
        for (let str of args.join(' ').split('$').values()) {
            if (str.startsWith('title')) {
                str = str.toString().replace('title', '').trim()
                embed.setTitle(`${str}`)
            } else if (str.startsWith('description')) {
                str = str.toString().replace('description', '').trim()
                embed.setDescription(`${str}`)
            } else if (str.startsWith('field')) {
                let field = str.replace('field', '').split('&&')
